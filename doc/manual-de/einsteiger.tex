...
\T\\
  \small Werner Koch}
%\date{Version 2.0.0.RC2 vom 7. März 2006}
\date{Version 2.0.0.RC2 vom 7. März 2006 und evtl. seitdem weiter bearbeitet}
...
OpenPGP Unterstützung in anderen Mailprogrammen.  GPGol funktioniert
...
Die Installation wird duch den Installer vorgenommen.  Beim nächsten
...
speichert Attachments nach der Entschlsselung im Klartext ab, so daß
...
dem Signieren des Haupttexts auch die Attachments signiert werden.
Dazu wird für jedes Attachment eine weiteres Attachment mit der
Signatur angelegt.
...
eigenen Schlüssels eintrage, so werden Ihre Nachrichten automatisch
auch an sie selbst verschlüsselt. Dies ermöglich die Nachricht dann
selber noch im Order für gesendete Nachrichten lesen zu können.
...
\item[Die Icons zur Verschlüsselung fehlen in der Symbolleiste] 
  ...
  Aufrufe der Schlüsselverwaltung.

\item[Was bedeuten die Buchstaben im Übersichtsdialog?]

  Bei der Entschlüsselung oder Signaturprüfung von Nachrichten mit
  Attachments zeigt GPGol eine Liste mit den einzelnen Besandteilen
  und deren Dateinamen dar. Vor dem Dateinamen steht eine "`E"' falls
  es sich um ein verschlüsseltes Attachment handelt oder eine "`S"'
  für ein signiertes Attachment.
...
\item[Warum unterstützt GPGol keine PGP/MIME Signaturen?]
  ...
  Durch die Architektur von Outlook ist es nicht möglich derartige
  Signaturen in in einem Plugin zu prüfen.  Outlook verändert die
  Nachrichten direkt beim Empfang, wodurch diese Signaturen nicht mehr
  gültig wären.
