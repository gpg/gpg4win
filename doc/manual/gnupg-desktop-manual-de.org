...
file into the empty Emailwindow). Add the recipient (adele@gnupp.de)
...
Table [\ref{table2}]table2 shows the exact allocation of the 77
...
Table[\ref{table1}]table1. An example: 60 is transformed into
the number in row 60 and column 0. Hence 60 is transformed to 25.

This is not surprising, as if we assume that using
Table[\ref{table1}]table1 for the conversion of 25 results in
60, it only stands to reason that we should obtain 25 for the
transformation of 60 when using Table [\ref{table2}]table2. In
...
[\ref{table1}]table1 and[\ref{table2}]table2,
...
2. This representation, for example using a basis of $2^2=4$ (instead of
   ...
   so that you can use the Table[\ref{table1}]table1 for
...
3. For encryption purposes, you can now use
   Table[\ref{table1}]table1 from page \pageref{table1}, which
   ...
   table[\ref{table1}]table1. The message now turns into the
...
4. Converted back to a basis of 4, the message becomes $311_4,
   ...
   53, 63, 50 using Table[\ref{table2}]table2 to obtain the
   sequence 4, 35, 8, which precisely corresponds with the original
   message.

   Using Tables [\ref{table1}]table1
   and[\ref{table2}]table2 you can also encrypt messages using
   the private key (e.g. first use Table[\ref{table2}]table2
   and then decode with the public key (i.e.
   Table[\ref{table1}]table1) and thus restore your original
