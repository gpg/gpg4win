...
# Grab the version information for packages released with a KDE Plasma release
...
server=https://download.kde.org/stable/plasma
echo "server ${server}"

tmpdir=$(mktemp -d -t gen-kde-plasma.XXXXXXXXXX)

for package in $PACKAGES; do
    # Download packages over https now and verify that the signature matches
    tarfile="${package}-${fullversion}.tar.xz"
    tarfileurl="${server}/${fullversion}/${tarfile}"
...
# verified: PGP Signed by ./kde-release-keys.gpg (created by gen-kde-plasma.sh)
file ${fullversion}/${tarfile}
chk $sha2
